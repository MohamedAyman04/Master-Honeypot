...
def inspect_request(ip, user_agent, username, password=None):
    ...
    # 3. Payload Injection Signatures (SQLi & Path Traversal)
    combined_payload = f"{username or ''} {password or ''}".lower()
    for pattern in SQLI_PATTERNS:
        if re.search(pattern, combined_payload, re.IGNORECASE):
            _quarantined_ips[ip] = {
                "reason": "SQL injection attempt detected in authentication payload",
    ...
    for pattern in TRAVERSAL_PATTERNS:
        if re.search(pattern, combined_payload, re.IGNORECASE):
            _quarantined_ips[ip] = {
                "reason": "Path traversal pattern detected in payload",
                "tactic": "T1190 (Exploit Public-Facing Application)",
                "timestamp": time.time()
            }
            return True, _quarantined_ips[ip]["reason"], _quarantined_ips[ip]["tactic"]
